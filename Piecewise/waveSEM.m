...
% Solves the wave equation using Legedre + weak Galerkin
...
B(1:end-2, 1:N)=K1(con,:);
B(1:end-2, 2*N+1:3*N)=-M1(con,:);
...
M2=E1'*kron(eye(3),M1)*E1;
K2=E1'*(kron(maskM1,M1)+kron(maskK1,K1))*E1;
A=M2\K2;
U=expm(A*dt);

figure(4);
imagesc(log(abs(K2))); colormap(gray(256)); colorbar();
