...
function [rhs]=schurComplement(b)
    ...
    w1=poissonSquare(D2(2:N-1,1)*b2+b1*D2(2:N-1,1)');
    w2=poissonSquare(b1*D2(2:N-1,N)');
    w3=poissonSquare(D2(2:N-1,N)*b2);
    
    r1=(D(end)-D(1))*b1-(w2*D(N,2:N-1)'-w1*D(1,2:N-1)');
    r2=(D(end)-D(1))*b2-(D(N,2:N-1)*w3-D(1,2:N-1)*w1);
    rhs=[r1(:); r2(:)];
end
