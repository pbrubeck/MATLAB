function [omega,G,F,eta,xi,h,indx,indy,modes,cptime] = ellipse_eigs(mode,alpha,beta,m,n,opts)
...
%              default is the superior type of input data (alpfa,beta)
...
if isfield(opts,'fp_type') && is_numeric_type_supported(opts.fp_type)  
    ...
    class_t = superiorfloat(alpha,beta);
end
opts.fp_type = class_t;

% Make sure all inputs are of the same numeric type.
if ~isa(alpha,class_t), alpha = numeric_t(alpha,class_t); end;
...
% Pi solutions
% ----------------------------------------------------------------------
[A1,B1,C1,A2,B2,C2,eta,xi,G1,k1,r1,G2,k2,r2] = mathieu_mep(n1,n2,premik+1,alpha,beta,opts);
[lambda,mu,X1,X2] = twopareigs(A1+5*B1,B1,C1,A2+5*B2,B2,C2,k,opts); % we shift so that A1 and A2 are not singular
...
% 2*Pi solutions
% ----------------------------------------------------------------------
[A1,B1,C1,A2,B2,C2,eta,xi,G1,k1,r1,G2,k2,r2] = mathieu_mep(n1,n2,premik+2,alpha,beta,opts); 
[lambda,mu,X1,X2] = twopareigs(A1,B1,C1,A2,B2,C2,k,opts);
XR1 = recover_bc(X1,G1,k1,r1);
XR2 = recover_bc(X2,G2,k2,r2);
for j=1:k
