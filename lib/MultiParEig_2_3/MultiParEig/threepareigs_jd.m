...
%   - delta : convergence criterion (norm of the residual) for the outer iteration (eps*max inf norm ot the matrices)
...
%   - maxsize : maximum dimension of search spaces before restart (6)
%   - maxmaxsize : maximum dimension of searh space including temporary increment in case of no convergence
%   - minsize : dimension of search spaces after restart (4)
...
%   - innersteps : number of GMRES steps for the correction equation (0)
...
% Fixed bug in the preconditioned first order correction 
...
if isfield(opts,'delta')
    ...
    mnor = max([norm(A1,'inf') norm(B1,'inf') norm(C1,'inf') norm(D1,'inf') ...
...
if isfield(opts,'minsize'),     minsize = getfield(opts,'minsize');        else minsize = 4;             end
if isfield(opts,'maxsize'),     maxsize = getfield(opts,'maxsize');        else maxsize = 6;             end
if isfield(opts,'maxmaxsize'),  maxmaxsize = getfield(opts,'maxmaxsize');  else maxmaxsize = 9;          end
...
if isfield(opts,'refine'),      refine = getfield(opts,'refine');          else refine = 3;              end
if isfield(opts,'solveropts'),  solveropts = getfield(opts,'solveropts');  else solveropts = [];              end

if ~isfield(opts,'solveropts.usesparse'),  solveropts.usesparse = 0; end;
...
while (conv<neig) && (step<maxsteps)
    ...
    while (conv<neig) && noexpansion
       ...
       switch extraction
           ...
           case 'mineta'  % Ritz value with the smallest |eta|
               [tilda,order] = sort(abs(Ae-target(3)));
        ...
        if (tmpres(step)<=switcheps) && (expandrandom==0) % candidate for convergence 
            ...
            if refine>0
                [la,ua,ea,Xrapr,Yrapr,Zrapr] = trqi_3p(A1,B1,C1,D1,A2,B2,C2,D2,A3,B3,C3,D3,Xrapr,Yrapr,Zrapr,refine,delta);
                ABC1 = A1-la*B1-ua*C1-ea*D1;
                ABC2 = A2-la*B2-ua*C2-ea*D2;
                ABC3 = A3-la*B3-ua*C3-ea*D3;
            end
            ...
            if (resnorm<delta) && (distance<minInnerProd)
                ...
                if showinfo>0
                    disp(sprintf('Eig (%2d): lambda: %+5.4e %+5.4ei, mu: %+5.4e %+5.4ei, eta: %+5.4e %+5.4ei, step %4d, res: %5.1e, selcrit: %5.1e',conv,real(la),imag(la),real(ua),imag(ua),real(ea),imag(ea),step,resnorm,minInnerProd))
                end
            else
                ...
                noexpansion = 0;
                if (mod(hist.missedTRQI,3)==1) && (switcheps>500*delta)
  ...
  if (conv<neig) && (step<maxsteps)
     ...
     % Additional directions are taken from last U1
