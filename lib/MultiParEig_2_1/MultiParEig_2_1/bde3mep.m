function [z,A,B,C,D,G,kd,rd] = bde3mep(a,b,p,q,r,s,t,u,bc,N)

%BDE3MEP Discretizes 3-parameter BDE using Chebyshev collocation
%
% [z,A,B,C,D,G,kd,rd] = BDE3MEP(a,b,p,q,r,s,t,u,bc,N) discretizes a 3-parameter BDE
% 
...
[x,tmpD] = chebdif(N,2); % attention - nodes in chebdif go from 1 to -1 
D0 = eye(N);
D1 = tmpD(:,:,1);
D2 = tmpD(:,:,2);
...
if isa(u,'function_handle'), diagU = diag(u(z)); else diagU = u*eye(N); end
