function [order,clstart,clsize,newx] = clusters(x,eps)
...
%
% x(i) and x(j) are in the same cluster if abs(x(i)-x(j))/(1+abs(x(i))) <= eps
% Output:
...
n = size(x,1);
dif = abs((x*ones(1,n)-ones(n,1)*x.')/(diag(1+abs(x))))<eps;
...
while any(free)
   cluster = [];
   [tilda1,tilda2,elem] = find(free);
   newindices = elem(1);
   while ~isempty(newindices)
	  free(newindices) = 0;
   	  cluster = [cluster newindices];
