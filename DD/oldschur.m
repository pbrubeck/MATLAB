...
function [s, h]=schurComplement(b)		
    ...
    for j=1:size(net,1)		
        if(any(any(b(:,net(j,:)))))
            bb=zeros(N,N);
            bb(rd,kd)=b(:,net(j,1:2))';
            bb(kd,rd)=b(:,net(j,3:4));
            w{j}=greenF(D2(kd,:)*bb(:,kd)+bb(kd,:)*D2(kd,:)');	
        else		
            w{j}=0;		
        end		
    end		
...
function [u]=poissonTiles(F)		
    ...
    for j=1:size(net,1)		
        ...
        bb(rd,kd)=b(:,net(j,1:2))';
        bb(kd,rd)=b(:,net(j,3:4));
        u(:,:,j)=greenF(F(:,:,j)-(D2(kd,:)*bb(:,kd)+bb(kd,:)*D2(kd,:)'));	
    end	
